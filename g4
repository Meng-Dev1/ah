...
local function MakeDraggable(topbarobject, object)
    local function CustomPos(topbarobject, object)
        ...
        local function UpdatePos(input)
            if isDraggingLocked then return end  -- lock check
...
function Menghub:Window(GuiConfig)
                            ...
                            for _, item in pairs(sectionAdd:GetChildren()) do
                                if (item:IsA("Frame") or item:IsA("TextButton")) and item.Name ~= "SubSection" and item.Name ~= "Divider" then
                                    ...
                                    if titleObj and string.find(string.lower(titleObj.Text), query, 1, true) then
                                        ...
                                        SubTitle.Text = tabName .. " • " .. sectionTitleText
    ...
    Close.Activated:Connect(function()
        ...
        Yes.MouseButton1Click:Connect(function()
            ...
            if game.CoreGui:FindFirstChild("LockGui") then  -- tambah ini
                game.CoreGui.LockGui:Destroy()
            end
    ...
    if isMobile then
        ...
        LockBtn.Position = UDim2.new(0, 20, 0, viewport.Y * 0.2)
        LockBtn.Size = UDim2.new(0, 90, 0, 36)
        LockBtn.BackgroundColor3 = Color3.fromRGB(20, 20, 26)
        ...
        LockBtn.Active = true -- PENTING: Mencegah input nembus ke belakang
        ...
        LockStroke.Transparency = 0.75
        LockStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
        LockStroke.Parent = LockBtn

        -- draggable
        local lbDragging = false
        local dragInput = nil
        local lbDragStart, lbStartPos
        local lbMoved = false

        LockBtn.InputBegan:Connect(function(inp)
            -- Hanya proses jika itu sentuhan layar atau klik mouse
            if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
                -- FILTER 1: Pastikan ini SENTUHAN BARU, bukan jari dari analog yang cuma lewat
                if inp.UserInputState == Enum.UserInputState.Begin then
                    
                    -- FILTER 2: Blokir kalau sudah ada jari lain yang lagi nge-drag (multi-touch lock)
                    if lbDragging then return end 

                    lbDragging = true
                    lbMoved = false
                    dragInput = inp
                    lbDragStart = inp.Position
                    lbStartPos = LockBtn.Position
        ...
        UserInputService.InputChanged:Connect(function(inp)
            -- Hanya jalan kalau inputnya sama persis dengan jari yang neken di awal
            if lbDragging and inp == dragInput then
                local delta = inp.Position - lbDragStart
                if math.abs(delta.X) > 5 or math.abs(delta.Y) > 5 then
                    lbMoved = true
                end
                LockBtn.Position = UDim2.new(
                    lbStartPos.X.Scale,
                    lbStartPos.X.Offset + delta.X,
                    lbStartPos.Y.Scale,
                    lbStartPos.Y.Offset + delta.Y
                )
            end
        end)
        ...
        local function SetLockState(locked)
            ...
            if locked then
                LockBtn.Text = "UNLOCK"
                LockBtn.TextColor3 = GuiConfig.Color
                LockStroke.Color = GuiConfig.Color
                LockStroke.Transparency = 0.3
                LockBtn.Size = UDim2.new(0, 110, 0, 36)
            else
                LockBtn.Text = "LOCK"
                LockBtn.TextColor3 = Color3.fromRGB(180, 180, 180)
                LockStroke.Color = Color3.fromRGB(255, 255, 255)
        ...
        LockBtn.MouseButton1Click:Connect(function()
            if lbMoved then return end
            SetLockState(not isDraggingLocked)
        end)
    end
        ...
        function Sections:AddSection(Title, AlwaysOpen)
            ...
            function Items:AddToggle(ToggleConfig)
                ...
                if ToggleConfig.Keybind then
                    ...
                    -- Fungsi Set untuk Keybind (Daftarin ke Elements)
                ...
                if ToggleConfig.Colorpicker then
                    ...
                    -- Kotak kecil warna di RightSide
                    ...
                    -- Popup di-parent ke ScreenGui biar ga kena ClipsDescendants
                    ...
                    local cpPopCorner = Instance.new("UICorner")
                    cpPopCorner.CornerRadius = UDim.new(0, 8)
                    cpPopCorner.Parent = CPPopup
                    local cpPopStroke = Instance.new("UIStroke")
                    cpPopStroke.Color = Color3.fromRGB(255, 255, 255)
                    cpPopStroke.Thickness = 1
                    cpPopStroke.Transparency = 0.8
                    cpPopStroke.Parent = CPPopup

                    -- Hue gradient bar (vertikal, kanan) — pure UIGradient, no assets
                    ...
                    -- SV picker — pure UIGradient berlapis
                    -- Layer 1: base hue solid
                    -- Layer 2: putih→transparan (kiri ke kanan = saturation)
                    -- Layer 3: transparan→hitam (atas ke bawah = value)
                    ...
                    -- Layer saturation (putih kiri → transparan kanan)
                    ...
                    -- Layer value (transparan atas → hitam bawah)
                    ...
                    SVCursor.ZIndex = 204  -- di atas semua gradient layer (201,202,203)
                    ...
                    HexLabel.Size = UDim2.new(0, 95, 0, 22)
                    HexLabel.ZIndex = 201
                    ...
                    -- posisi popup: tepat di bawah ColorBox, di screen space
                    local function UpdatePopupPosition()
                        local abs = ColorBox.AbsolutePosition
                        local popW, popH = 180, 215
                        local vpX, vpY = viewport.X, viewport.Y
                        local px = abs.X - popW + 20
                        local py = abs.Y + 26
                        -- jangan keluar layar bawah
                        if py + popH > vpY - 10 then py = abs.Y - popH - 4 end
                        if px < 4 then px = 4 end
                        CPPopup.Position = UDim2.new(0, px, 0, py)
                    end

                    -- Dragging SV — fix: .X/.Y bukan Vector3
                    ...
                    -- Dragging Hue — fix: .X/.Y bukan Vector3
                    ...
                    -- Backdrop full-screen: nyerap semua klik di luar popup
                    local CPBackdrop = Instance.new("Frame")
                    CPBackdrop.Name = "CPBackdrop"
                    CPBackdrop.BackgroundTransparency = 1
                    CPBackdrop.Size = UDim2.new(1, 0, 1, 0)
                    CPBackdrop.Position = UDim2.new(0, 0, 0, 0)
                    CPBackdrop.ZIndex = 199
                    CPBackdrop.Visible = false
                    CPBackdrop.Parent = cpScreenGui

                    -- Tutup popup hanya kalau klik di luar (bukan drag release)
                    ...
                    UserInputService.InputChanged:Connect(function(inp)
                        if inp.UserInputType == Enum.UserInputType.MouseMovement and (svDragging or hueDragging) then
                            wasDragging = true
                        end
                    end)
                    UserInputService.InputEnded:Connect(function(inp)
                        ...
                        -- cek apakah posisi klik di luar popup
                ...
                local function UpdateHeight()
                    ...
                    Toggle.Size = UDim2.new(1, 0, 0, contentHeight + 35)
                    if UpdateSizeSection then UpdateSizeSection() end
                end
                ToggleContent:GetPropertyChangedSignal("TextBounds"):Connect(UpdateHeight)
                UpdateHeight()
            ...
            function Items:AddKeybind(KeybindConfig)
                ...
                UserInputService.InputBegan:Connect(function(input, gpe)
                    ...
                    elseif input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == KeybindFunc.Value then
                        if KeybindConfig.Premium and not CheckIsPremium() then return end
            ...
            function Items:AddCheckbox(CheckboxConfig)
                ...
                local function UpdateHeight()
                    local contentHeight = CheckboxContent.TextBounds.Y
                    Checkbox.Size = UDim2.new(1, 0, 0, contentHeight + 38)
                    if UpdateSizeSection then UpdateSizeSection() end
                end
                CheckboxContent:GetPropertyChangedSignal("TextBounds"):Connect(UpdateHeight)
                UpdateHeight()
